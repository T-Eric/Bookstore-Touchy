...
void BookMaster::stringToArray(const string& str, array<char, 64>& arr) {
  arr.fill(0);
...
bool BookMaster::preModify(const string& _isbn, const string& _name, const string& _author, const string& _keys, double _price) {
  ...
  if (_keys != "") {
    stringToArray(_keys, temparray);
    if (!cutKey(temparray, keyBusket))
      return false;
...
bool BookMaster::Modify(bool isisbn, bool isname, bool isauthor, bool iskeyword, bool isprice) {
  // 如果修改的isbn之前已经有了，那么返回错误
  if (Search(arrayToChar(modified.isbn), ISBN))
    return false;
