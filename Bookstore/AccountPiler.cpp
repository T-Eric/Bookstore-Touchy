...
bool AccountStack::comp_strarr(const string& str, const array<char, 32>& arr) {
  int i;
  for (i = 0; i < std::min(str.size(), arr.size()); i++) {
    if (arr[i] != str[i])
      return false;
  }
  if (arr[i] != 0)
...
// 抽出来，与旧的头部一样的，全部换新的
void AccountStack::modifySelected(const BookInfo& info) {
  tempinfo = Bookstack.top();  // 旧
  if (info.isbn == tempinfo.isbn)
    return;  // isbn号没变就无所谓
  while (!Bookstack.empty()) {
    tempstack.push(Bookstack.top());
    Bookstack.pop();
    if (tempstack.top().isbn == tempinfo.isbn) {
      tempstack.top().isbn = info.isbn;
    }
  }
...
void AccountPiler::Init() {
  active = Account("temp", "temp", "temp", 9);
...
bool AccountPiler::revisePwd(const string& _uid, const string& newpwd, const string& curpwd) {
  if (active.privilege >= 1 && findAccount(_uid.c_str())) {
    if (active.privilege == 7 || comp_strarr(curpwd, retacc[0].password)) {
